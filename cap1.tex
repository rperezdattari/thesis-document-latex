...
A Markov Decision Process (MDP) is meant to be a straightforward framing of sequential decision-making problems. In an MDP we have an \emph{agent} and an \emph{environment} which interact continually. The agent is the learner and the decision maker. The environment is everything that the agent can interact with. In robotics, the agent is the robot and the environment is the real world setting in which the robots exists.
...
The environment's evolution is the sequence of states visited by the agent. Transitioning from one state to another is a stochastic process defined by the nature of the environment and the actions taken, at each time step, by the agent. Additionally, an MDP is formulated as an extension of a Markov chain, such that the Markov property is satisfied. Consequently,the conditional probability distribution of future states depends only upon the present state, such that:
...
\begin{figure}[H]
    ...
    \label{fig:msim}
\end{figure}
\footnotetext{Image taken from \url{https://artint.info/html/ArtInt_224.html}.}

\section{Reinforcement Learning \cite{sutton2018reinforcement, kober2013reinforcement}}
...
\end{figure}
\footnotetext{Image taken from \cite{sutton2018reinforcement}.}
...
\begin{figure}[h]
    ...
    \label{fig:rl_summ}
\end{figure}
\footnotetext{Image taken from \url{http://www0.cs.ucl.ac.uk/staff/d.silver/web/Teaching_files/pg.pdf}}
...
\end{figure}
\footnotetext{Image taken from \url{http://rail.eecs.berkeley.edu/deeprlcourse/static/slides/lec-4.pdf}}
...
\end{figure}
\footnotetext{Image taken from \url{http://www0.cs.ucl.ac.uk/staff/d.silver/web/Teaching_files/dyna.pdf}}
...
Where $N$ is the number of basis functions and the sub index $i$ refers to the i-th basis function.
...
\begin{figure}[H]
    ...
    \label{fig:neuron}
\end{figure}
\footnotetext{Image taken from \url{https://becominghuman.ai/artificial-neuron} \newline \url{-networks-basics-introduction-to-neural-networks-3082f1dcca8c}}

As mentioned before, a layer may have many neurons. The number of neurons in a layer is known as the \emph{width} of the layer. The inputs $x$ of a neuron are the outputs of every neuron of the past layer.  There are three types of layers: \textbf{input}, \textbf{hidden} and \textbf{output}. The input layer is the input of $\Psi$, which is $s$ in this context (which is not actually a layer). The output layer is the final function in the chain of functions that compose $\Psi$, which outputs the output of $\Psi$. And finally, the hidden layers are all the functions in between the input and output layers. Graphically, a FNN with one hidden layer is presented in Figure \ref{fig:FNN}, where every layer is denoted by $u_{j}$.
...
\begin{figure}[H]
    ...
    \label{fig:FNN}
\end{figure}
\footnotetext{Image taken from \url{http://lenguyenthedat.com/minimal-data-science-3-mnist-neuralnet/}}
...
\begin{figure}[H]
    ...
    \label{fig:conv}
\end{figure}
\footnotetext{Image taken from \url{https://developer.apple.com/library/archive/documentation/} \newline \url{Performance/Conceptual/vImage/ConvolutionOperations/ConvolutionOperations.html}}
...
\begin{figure}[H]
    ...
    \label{fig:maxpool}
\end{figure}
\footnotetext{Image taken from \url{https://computersciencewiki.org/index.php/Max-pooling_/_Pooling}}
...
\label{fig:cnn}
\end{figure}
\footnotetext{Image taken from \url{https://towardsdatascience.com/build-your-own} \newline \url{-convolution-neural-network-in-5-mins-4217c2cf964f}}
...
\begin{figure}[H]
    ...
    \label{fig:ae}
\end{figure}
\footnotetext{Image taken from \url{https://hackernoon.com/latent-space-visualization-deep-learning-bits-2-bd09a46920df}}    
...
\end{figure}
\footnotetext{Image taken from \url{http://blog.jacobandreas.net/monference.html}}
...
\begin{figure}[h]
    ...
    \label{fig:b_cloning}
\end{figure}
\footnotetext{Image taken from \url{http://rail.eecs.berkeley.edu/deeprlcourse/static/slides/lec-2.pdf}}
